...
const JsonToCsv = () => {
    ...
    const csvLineNumbersRef = useRef(null);

    const handleConvertToCsv = () => {
        try {
            const parsedJson = JSON.parse(input);
            if (!Array.isArray(parsedJson)) {
                throw new Error('JSON input should be an array of objects.');
            }
            ...
            setCsvOutput(`Invalid JSON: ${error.message}`);
        }
    };
    ...
    useEffect(() => {
        ...
        if (csvOutputRef.current) {
            csvOutputRef.current.scrollTop = csvOutputRef.current.scrollHeight;
        }
    }, [input, csvOutput]);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            syncScroll(csvOutputRef, csvLineNumbersRef);
        };
        ...
        if (csvOutputRef.current) {
            csvOutputRef.current.addEventListener('scroll', handleScroll);
        }
        ...
        return () => {
            ...
            if (csvOutputRef.current) {
                csvOutputRef.current.removeEventListener('scroll', handleScroll);
            }
        };
    }, []);
    ...
    useEffect(() => {
        ...
        if (csvLineNumbersRef.current) {
            csvLineNumbersRef.current.innerText = addLineNumbers(csvOutput);
        }
    }, [input, csvOutput]);
        ...
        <div className="h-screen flex flex-row">
            ...
            <div className='sticky w-full flex flex-col bg-black h-full'>
                <div className='flex justify-center items-center py-5 bg-black w-full'>
                    <h1 className='text-white text-xl text-center'>JSON to CSV Converter</h1>
                </div>
                ...
                <div className='flex flex-row h-full'>
                    ...
                    <div className='w-2/4 flex-grow bg-white/20 pt-2'>
                        <div className='relative h-[95%] ml-10 px-2 pr-6'>
                            <h1 className='text-xs p-[2px] text-white/50 font-thin text-center'>Your CSV code here.</h1>
                            <div className='bg-[#0F172A] rounded-2xl z-20 h-full p-4 flex'>
                                <div className='line-numbers pr-2 text-gray-500 bg-[#0e1424] overflow-hidden' ref={csvLineNumbersRef}></div>
                                <pre
                                    ref={csvOutputRef}
                                    className="w-full overflow-y-scroll scrollbar-hide text-white bg-[#0e1424] rounded-lg px-2 outline-none h-full"
                                    readOnly
                ...
                <div className='absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 h-fit w-fit'>
                    <div className='  bg-red-600 p-1.5 rounded-full'>
                        <button type="button" className='button' onClick={handleConvertToCsv}>
                            <svg
