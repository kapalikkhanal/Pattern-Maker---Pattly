...
const JsonToXml = () => {
    ...
    const convertedXmlRef = useRef(null);
    ...
    const handleConvertToXml = () => {
        try {
            ...
            let xml = json2xml(parsedJson, xmlOptions);
            // Enclose the XML with the declaration and a root element
            xml = `<?xml version="1.0" encoding="UTF-8"?>\n<root>\n${xml}\n</root>`;
            setConvertedXml(xml);
        } catch (error) {
            setConvertedXml('Invalid JSON');
        }
    };
  ...
  "employees": {
    ...
    useEffect(() => {
        ...
        if (convertedXmlRef.current) {
            convertedXmlRef.current.scrollTop = convertedXmlRef.current.scrollHeight;
        }
    }, [input, convertedXml]);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            syncScroll(convertedXmlRef, convertedLineNumbersRef);
        };
        ...
        if (convertedXmlRef.current) {
            convertedXmlRef.current.addEventListener('scroll', handleScroll);
        }
        ...
        return () => {
            ...
            if (convertedXmlRef.current) {
                convertedXmlRef.current.removeEventListener('scroll', handleScroll);
            }
        };
    }, []);
    ...
    useEffect(() => {
        ...
        if (convertedLineNumbersRef.current) {
            convertedLineNumbersRef.current.innerText = addLineNumbers(convertedXml);
        }
    }, [input, convertedXml]);
        ...
        <div className="h-screen flex flex-row">
            ...
            <div className='sticky w-full flex flex-col bg-black h-full'>
                <div className='flex justify-center items-center py-5 bg-black w-full'>
                    <h1 className='text-white text-xl text-center'>JSON to XML</h1>
                </div>
                ...
                <div className='flex flex-row h-full'>
                    ...
                    <div className='w-2/4 flex-grow bg-white/20 pt-2'>
                        <div className='relative h-[95%] ml-10 px-2 pr-6'>
                            <h1 className='text-xs p-[2px] text-white/50 font-thin text-center'>Your converted XML here.</h1>
                            <div className='bg-[#0F172A] rounded-2xl z-20 h-full p-4 flex'>
                ...
                <div className='absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 h-fit w-fit'>
                    <div className='  bg-red-600 p-1.5 rounded-full'>
                        <button type="button" className='button' onClick={handleConvertToXml}>
                            <svg
